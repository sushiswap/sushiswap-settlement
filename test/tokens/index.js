...
module.exports = {
    WETH: {
        ..._WETH,
        31337: new Token(31337, "0x128236dc0cF966F37E4843D7Ed8E09b41B4053F8", 18, "WETH", "Wrapped Ether")
    },
    DAI: [1, 4, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, dai[current], 18, "DAI", "Dai Stablecoin")
    }), {}),
    USDC: [1, 4, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, usdc[current], 6, "USDC", "USD//C")
    }), {}),
    USDT: [1, 4, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, usdt[current], 6, "USDT", "Tether USD")
    }), {}),
    COMP: [1, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, comp[current], 18, "COMP", "Compound")
    }), {}),
    MKR: [1, 4, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, mkr[current], 18, "MKR", "Maker")
    }), {}),
    OMG: [1, 4, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, omg[current], 18, "OMG", "OMG Network")
    }), {}),
    BAT: [1, 4, 42, 31337].reduce((prev, current) => ({
        ...prev,
        [current]: new Token(current, bat[current], 18, "BAT", "BAT")
    }), {}),
    SUSHI: {
        [ChainId.MAINNET]: new Token(ChainId.MAINNET, "0x6B3595068778DD592e39A122f4f5a5cF09C90fE2", 18, "SUSHI", "SushiSwap")
    },
    YAM: {
        [ChainId.MAINNET]: new Token(ChainId.MAINNET, "0x0e2298E3B3390e3b945a5456fBf59eCc3f55DA16", 18, "YAM", "YAM")
    },
    AMPL: {
        [ChainId.MAINNET]: new Token(ChainId.MAINNET, "0xD46bA6D942050d489DBd938a2C909A5d5039A161", 9, "AMPL", "Ampleforth")
    }
};
