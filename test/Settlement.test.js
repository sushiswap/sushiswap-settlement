...
describe("Settlement", function () {
    ...
    it("Should revert fillOrder() if called with invalid args", async () => {
        const { chainId, users, addLiquidity, getTrade, createOrder, fillOrder, getDeadline } = await helpers.setup();
        const fromToken = WETH[chainId];
        const toToken = DAI[chainId];
        ...
        await helpers.expectToBeReverted(
            "invalid-maker",
            fillOrder(users[1], order, trade, { maker: ethers.constants.AddressZero })
        );
        await helpers.expectToBeReverted(
            "invalid-from-token",
            fillOrder(users[1], order, trade, { fromToken: ethers.constants.AddressZero })
        );
        await helpers.expectToBeReverted(
            "invalid-to-token",
            fillOrder(users[1], order, trade, { toToken: ethers.constants.AddressZero })
        );
        await helpers.expectToBeReverted(
            "duplicate-tokens",
            fillOrder(users[1], order, trade, { toToken: fromToken.address })
        );
        await helpers.expectToBeReverted(
            "invalid-amount-in",
            fillOrder(users[1], order, trade, { amountIn: ethers.constants.Zero })
        );
        await helpers.expectToBeReverted(
            "invalid-amount-out-min",
            fillOrder(users[1], order, trade, { amountOutMin: ethers.constants.Zero })
        );
        await helpers.expectToBeReverted(
            "invalid-recipient",
            fillOrder(users[1], order, trade, { recipient: ethers.constants.AddressZero })
        );
        await helpers.expectToBeReverted(
            "invalid-deadline",
            fillOrder(users[1], order, trade, { deadline: ethers.constants.Zero })
        );
        await helpers.expectToBeReverted(
            "invalid-amount-to-fill-in",
            fillOrder(users[1], order, trade, { amountToFillIn: ethers.constants.Zero })
        );
        await helpers.expectToBeReverted(
            "invalid-path-length",
            fillOrder(users[1], order, trade, { path: [ethers.constants.AddressZero] })
        );
        await helpers.expectToBeReverted(
            "invalid-path-length",
            fillOrder(users[1], order, trade, { path: [ethers.constants.AddressZero] })
        ...
        await helpers.expectToBeReverted(
            "invalid-path-start",
            fillOrder(users[1], order, trade, { path: [ethers.constants.AddressZero, toToken.address] })
        );
        await helpers.expectToBeReverted(
            "invalid-path-end",
            fillOrder(users[1], order, trade, { path: [fromToken.address, ethers.constants.AddressZero] })
        );
    });
